...
이 두 값이 없어도 관리자 앱 배포 자체는 가능합니다. 다만 `/signin` 화면은 Google 로그인 버튼 대신 설정 필요 안내를 보여줍니다. 실제 관리자 로그인을 완성하는 마지막 단계가 이 Google OAuth 설정입니다.
...
이 상태는 배포 실패가 아니라 설정 대기 상태일 수 있습니다. `/signin` 화면에서 설정 필요 안내가 보이면 키트 껍데기는 정상 배포된 것입니다.
